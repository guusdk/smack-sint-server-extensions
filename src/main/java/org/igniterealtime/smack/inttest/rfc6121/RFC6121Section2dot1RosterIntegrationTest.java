...
public class RFC6121Section2dot1RosterIntegrationTest extends AbstractSmackIntegrationTest
{
    ...
    @SmackIntegrationTest(section = "2.1.5", quote = "The following rules apply to roster sets: [...] The server MUST ignore any value of the 'subscription' attribute other than \"remove\"")
    public void testRosterSetWithIrregularSubscriptionAttribute() throws XmppStringprepException, SmackException.NotConnectedException, SmackException.NoResponseException, InterruptedException, XMPPException.XMPPErrorException, SmackException.NotLoggedInException
    {
        // Setup test fixture
        final BareJid target = JidCreate.bareFrom( Localpart.from("test-target-" + StringUtils.randomString(5) ), conOne.getXMPPServiceDomain() );
        ...
        try {
            ...
            // Verify result
            assertEquals(IQ.Type.result, result.getType(), "Unexpected response type received by '" + conOne.getUser() + "' after it sent a Roster Set that included a subscription attribute with a value of 'foobar'. It was expected that the server would ignore this value.");

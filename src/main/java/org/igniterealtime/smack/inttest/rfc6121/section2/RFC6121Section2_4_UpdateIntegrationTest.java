...
public class RFC6121Section2_4_UpdateIntegrationTest extends AbstractSmackIntegrationTest
{
    private final boolean isSendPresence;

    private final SmackIntegrationTestEnvironment environment;

    public RFC6121Section2_4_UpdateIntegrationTest(SmackIntegrationTestEnvironment environment) throws SmackException.NotConnectedException, SmackException.NoResponseException, InterruptedException, TestNotPossibleException
    {
        super(environment);
    ...
    public void testRosterUpdateDeleteAllGroups() throws XmppStringprepException, XMPPException.XMPPErrorException, SmackException.NotConnectedException, SmackException.NoResponseException, InterruptedException, SmackException.NotLoggedInException
    {
        ...
        try {
            ...
            final RosterEntry rosterItem = roster.getEntry(target);
            assertEquals("Romeo", rosterItem.getName(), "Unexpected name for the roster item of '" + target + "' on the roster of '" + conOne.getUser() + "': Expected to name to be equal to that what was used in a Roster Update that intended to remove all groups from a preexisting roster item (but it was not).");
    ...
    public void testRosterUpdateChangeHandle() throws XmppStringprepException, XMPPException.XMPPErrorException, SmackException.NotConnectedException, SmackException.NoResponseException, InterruptedException, SmackException.NotLoggedInException
    {
        ...
        try {
            ...
            fail("Unexpected error response received by '" + conOne.getUser() + "' after it sent a a Roster Update that intended to update the handle a preexisting roster item.");
        } finally {
            // Tear down test fixture
            final RosterEntry entry = Roster.getInstanceFor(conOne).getEntry(target);
            if (entry != null) {
